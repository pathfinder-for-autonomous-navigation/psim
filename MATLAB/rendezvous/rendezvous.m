...
[~, r1, v1, r2, v2] = drift_phase(r1, v1, r2, v2, t_drift, 1);
...
[X, U] = l1cost(Qn, Q, alpha, A, B, N, [Q_transform * (r2 - r1); Q_transform * (v2 - v1)], rho, eps);
...
for i = 1:(N - 1)
    ...
    % Calculate new state
    Q_transform = eci_to_lvlh(r1, v1);
    X(:, i + 1) = [Q_transform * (r2 - r1); Q_transform * (v2 - v1)];
...
if plot_flag
    ...
    plot(t, dy(:, 6), '-b')
    hold off
    title('Drift Phase Relative Velocity of Satellite Two (rgb ~ xyz LVLH)')
    xlabel('Time (s)')
    ylabel('Velocity (m/s)')
