...
if __name__ == '__main__':
    ...
    parser.add_argument("-pa", "--port-adcs", action="store", help="Serial port to open to connect to the ADCS Controller.",   required=False)
    parser.add_argument("-pf", "--port-fc",   action="store", help="Serial port to open to connect to the Flight Controller.", required=False)
    ...
    with open("pan_logo.txt", "r") as pan_logo:
        if args.port_fc:
            fc_cmd = StateSession(args.data_dir, "FlightController")
            if not fc_cmd.connect(args.port_fc, 1152000):
                fc_cmd = None
        if args.port_adcs:
            adcs_cmd = StateSession(args.data_dir, "ADCSController")
