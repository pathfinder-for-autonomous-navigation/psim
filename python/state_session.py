...
class StateSession(object):
    ...
    def connect(self, console_port, baud_rate, msg_check_delay=0.02):
        ...
        try:
            ...
            self.teensy_time_lock = threading.Lock()       # Used to synchronize access of onboard-flight computer system time.
            self.current_teensy_time_raw = 0               # Raw time (in milliseconds) reported by Teensy during the last serial communication.
            self.current_teensy_time = 0                   # Current number of milliseconds since the serial connection was initiated, according
                                                           # to the Teensy.

            self.check_msgs_thread = threading.Thread(target=self.check_console_msgs, args=(msg_check_delay))
    ...
    def check_console_msgs(self, delay):
        ...
        while self.running_logger:
            ...
            try:
                ...
                # Update local understanding of time on the Teensy
                self.teensy_time_lock.acquire()
                old_teensy_time_raw = self.current_teensy_time_raw
                self.current_teensy_time_raw = data["t"] # Teensy time in milliseconds.
                self.current_teensy_time += self.current_teensy_time_raw - old_teensy_time_raw
                if old_teensy_time_raw > self.current_teensy_time_raw:
                    # The Teensy had an integer wrap-around. Compute the wrap-around and adjust the
                    # internal time appropriately.
                    self.current_teensy_time += (2**32 - 1)

                data["time"] = self.start_time + datetime.timedelta(milliseconds=self.current_teensy_time)
                self.teensy_time_lock.release()
                ...
                if "msg" in data:
                    ...
                    if data['field'] == self.awaited_value_name:
